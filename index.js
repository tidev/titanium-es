...
const api = JSON.parse(fs.readFileSync(process.argv.length > 2 ? process.argv[2] : 'api.jsca'));
...
for (const proxy of api.types) {
    ...
    const output = ejs.render(fs.readFileSync('ProxyTemplate.ejs').toString(), data);
...
const output = ejs.render(fs.readFileSync('IndexTemplate.ejs').toString(), { register });
const targetPath = path.join(outputDir, 'index.js');
fs.writeFileSync(targetPath, output);

// Copy over base proxy wrapper.
fs.copyFileSync('ProxyWrapper.js', path.join(outputDir, 'ProxyWrapper.js'));
